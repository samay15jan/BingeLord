...
Retrieves a list of recommended TV shows based on a selected series.

+++ Request
  HTTP
  ```http
    GET /api/recommendedTV?id={series_id}
  ```
  cURL
  ```sh
    curl -X GET "https://bingelord-backend.onrender.com/api/recommendedTV?id=12345"
  ```
+++ Response
  ```json
{
...
|-----------|---------------------------|---------|----------|
| [!badge variant="dark" text="id"] | Series ID to fetch recommendations. | None | [!badge variant="dark" text="true"] |
